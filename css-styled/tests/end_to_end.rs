...
fn full_css_output() {
    ...
    // Base rules
    assert!(
        css.contains(".split-handle {"),
        "missing scope rule in: {css}"
    );
    assert!(css.contains("display: flex"), "missing display in: {css}");
    assert!(
        css.contains("align-items: center"),
        "missing align-items in: {css}"
    );
    assert!(
        css.contains("flex-shrink: 0"),
        "missing flex-shrink in: {css}"
    );

    assert!(
        css.contains(".split-handle.horizontal {"),
        "missing horizontal in: {css}"
    );
    assert!(
        css.contains("cursor: col-resize"),
        "missing cursor in: {css}"
    );

    assert!(
        css.contains(".split-handle.horizontal .split-handle-bar {"),
        "missing h bar in: {css}"
    );
    assert!(css.contains("height: 100%"), "missing height in: {css}");

    assert!(
        css.contains(".split-handle.vertical {"),
        "missing vertical in: {css}"
    );
    assert!(
        css.contains("cursor: row-resize"),
        "missing cursor v in: {css}"
    );

    assert!(
        css.contains(".split-handle-bar {"),
        "missing bar rule in: {css}"
    );
    assert!(
        css.contains("pointer-events: none"),
        "missing pointer-events in: {css}"
    );

    // Dynamic rules
    assert!(
        css.contains(".split-handle { width: 8px; }"),
        "missing width in: {css}"
    );
    assert!(
        css.contains(".split-handle .split-handle-bar {"),
        "missing bar selector in: {css}"
    );
    assert!(css.contains("width: 2px"), "missing bar width in: {css}");
    assert!(
        css.contains("background: #1a1a1a"),
        "missing bar bg in: {css}"
    );
    assert!(
        css.contains(".split-handle:hover .split-handle-bar { background: #333; }"),
        "missing hover in: {css}"
    );
}
