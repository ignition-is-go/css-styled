...
fn color_values_valid() {
    ...
    assert_eq!(
        validate_value("background", "transparent"),
        ValidationResult::Valid
    );
    assert_eq!(
        validate_value("color", "currentColor"),
        ValidationResult::Valid
    );
    assert_eq!(validate_value("color", "red"), ValidationResult::Valid);
    assert_eq!(
        validate_value("background-color", "rgb(0, 0, 0)"),
        ValidationResult::Valid
    );
    assert_eq!(
        validate_value("color", "hsl(120, 100%, 50%)"),
        ValidationResult::Valid
    );
}
...
fn keyword_values_valid() {
    ...
    assert_eq!(
        validate_value("pointer-events", "none"),
        ValidationResult::Valid
    );
}
...
fn css_functions_pass_through() {
    assert_eq!(
        validate_value("width", "var(--custom)"),
        ValidationResult::Valid
    );
    assert_eq!(
        validate_value("width", "calc(100% - 20px)"),
        ValidationResult::Valid
    );
    assert_eq!(
        validate_value("width", "env(safe-area-inset-top)"),
        ValidationResult::Valid
    );
}
...
fn shorthand_values_accepted() {
    assert_eq!(
        validate_value("transition", "background 0.1s ease"),
        ValidationResult::Valid
    );
    assert_eq!(
        validate_value("border", "1px solid #333"),
        ValidationResult::Valid
    );
}
...
fn unknown_property_returns_valid() {
    assert_eq!(
        validate_value("not-a-property", "anything"),
        ValidationResult::Valid
    );
}
